...
define("common/lrc.js",["base/event"],function(t){function e(t){var e=t.split(/[:\.]/);return 60*parseInt(e[0].replace(o,"$1"))+parseInt(e[1].replace(o,"$1"))+parseInt((e[2]||"0").replace(o,"$1"))/1e3}function r(t){var r=[];return t.replace(u,function(t,n){r.push(e(n))}),r}function n(t){t=t||"";for(var e,n,i,u=t.split(s),o=[],f={},p=0,h=u.length;h>p;p++)if(u[p]=u[p].replace(l,""),i=u[p].match(c))f[i[1]]=i[2]||"";else if(i=u[p].match(a)){e=r(i[1]),n=u[p].replace(a,"");for(var d=0,m=e.length;m>d;d++)o.push({time:e[d],line:n,id:+new Date+String(Math.random()).replace(".","")})}return o.sort(function(t,e){return t.time-e.time}),{meta:f,lines:o}}var i=t("base/event"),s=/[\r\n]+/,c=/^\[(ti|ar|al|by|offset):([^\[\]]*)\]$/,l=/^\s+|\s+$/,a=/((?:\[\d{1,2}:\d{1,2}(?:[:\.]\d{1,3}?)\])(?:\s*\[\d{1,2}:\d{1,2}(?:[:\.]\d{1,3}?)\])*)/,u=/\[(\d{1,2}:\d{1,2}(?:[:\.]\d{1,3}?))\]/g,o=/^0+([0-9]+)$/,f=i.extend({propertys:function(){this.lrc="",this.lrcObj},initialize:function(t){this._setOption(t)},_setOption:function(t){t.lrc&&this.setLrc(t.lrc)},setLrc:function(t){this.lrcObj=n(t),this.emit("update",[this.lrcObj])},getLrc:function(){return this.lrcObj},pos:function(t){if(!this.lrcObj||!this.lrcObj.lines)return null;for(var e,r,n=this.lrcObj.lines,i={},s=0,c=n.length;c>s;s++)if(e=n[s],r=n[s+1]||i,e.time<=t&&(r===i||r.time>t)||0==s&&e.time>=t)return e;return null}});return f.parse=n,f});
